...
const TAG_ICON = { Hiring: 'briefcase', Skill: 'bolt', Tool: 'wrench', Salary: 'rupee', Opportunity: 'star' };
...
export default function NewsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="flex items-center gap-2 text-gold text-sm font-semibold"><Icon name="trend" size={17} /> Trend radar</div>
          ...
          {items.map((it, i) => (
            <div key={i} className="rounded-2xl border border-line/70 bg-card p-5">
              <div className="flex items-center gap-2">
                <span className={`text-[11px] px-2 py-0.5 rounded-full inline-flex items-center gap-1 ${TAG_COLOR[it.tag] || 'text-mut bg-ink'}`}><Icon name={TAG_ICON[it.tag] || 'star'} size={12} />{it.tag}</span>
              ...
              <button
                ...
                className="mt-3 text-sm text-brand hover:underline inline-flex items-center gap-1.5">
                <Icon name="bee" size={15} /> Ask Bee what this means →
              </button>
            </div>
          ))}
